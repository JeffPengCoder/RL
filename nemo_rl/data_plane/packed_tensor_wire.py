...
def packed_tensor_schema_from_extra_info(
    ...
    return validate_packed_tensor_wire_schema(
        extra_info[PACKED_TENSOR_WIRE_SCHEMA_KEY]
    )
...
def concat_packed_tensor_wire_schemas(
    ...
    *,
    ...
    merged_sample_ids = [
        sample_id for group in sample_id_groups for sample_id in group
    ]
    _validate_sample_ids(merged_sample_ids)

    first_entries = {
        entry["logical_key"]: entry for entry in validated[0]["entries"]
    }
    ...
    *,
    ...
    for entry in validated["entries"]:
        ...
        projected["row_sha256_by_sample_id"] = {
            sample_id: entry["row_sha256_by_sample_id"][sample_id]
            for sample_id in ids
        }
        projected["row_numel_by_sample_id"] = {
            sample_id: entry["row_numel_by_sample_id"][sample_id]
            for sample_id in ids
        }
...
def decode_packed_tensor_wire(
    ...
    *,
    ...
    for entry in validated["entries"]:
        ...
        if (
            shapes.shape[1] > rank
            and torch.count_nonzero(shapes[:, rank:]).item() != 0
        ):
