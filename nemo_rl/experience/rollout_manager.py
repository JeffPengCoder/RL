...
class RolloutManager:
    ...
    async def run_rollout(
        ...
        *,
        ...
        """Run one prompt group, optionally persisting completed Gym siblings."""
        ...
        if work.num_generations != self._num_generations_per_prompt:
            raise ValueError(
                "checkpoint work expects "
                f"{work.num_generations} generations, but RolloutManager "
                f"is configured for {self._num_generations_per_prompt}"
            )
    ...
    async def generate_and_push(
        ...
        *,
        ...
        if checkpoint_work is not None:
            ...
            if checkpoint_work.num_generations != self._num_generations_per_prompt:
                raise ValueError(
                    "checkpoint work expects "
                    f"{checkpoint_work.num_generations} generations, but "
                    "RolloutManager is configured for "
