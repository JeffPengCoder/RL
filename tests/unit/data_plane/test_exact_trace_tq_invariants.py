...
def test_runtime_transfer_queue_pip_patch_is_absent_and_reached_api_is_gated():
    ...
    assert "_EXPECTED_TRANSFER_QUEUE_COMMIT" not in adapter
    assert "_REQUIRED_TRANSFER_QUEUE_CALLABLES" in adapter
    assert "validate_transfer_queue_api()" in adapter
...
def test_multimodal_exact_uses_first_class_wire_and_cp_replica_transport():
    ...
    assert "encode_packed_tensor_wire" in _source("nemo_rl/data_plane/column_io.py")
