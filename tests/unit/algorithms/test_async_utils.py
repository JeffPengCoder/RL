...
class TestAsyncTrajectoryCollector:
    ...
    def test_process_batch_assigns_unique_context_compaction_replica_ids(
        self, monkeypatch
    ):
        """Each async generation replica receives its own retry-stable rollout ID."""
    ...
    def test_prompt_group_worker_records_async_policy_provenance(
        ...
        trajectory_group, generation_weight, target_weight = add_calls[0]
        assert generation_weight == 3
        assert target_weight == 4
        assert trajectory_group["generation_weight_version"] == 3
        assert trajectory_group["target_weight_version"] == 4
