...
def test_vllm_async_http_server_loads_reasoning_parser_plugin(monkeypatch):
    ...
    assert online_renderer.instances[0].kwargs["default_chat_template_kwargs"] == {
        "enable_thinking": False
    }
    assert serving_tokenization.instances[0].kwargs[
        "default_chat_template_kwargs"
    ] == {"enable_thinking": False}
    assert openai_serving_chat.instances[0].kwargs["trust_request_chat_template"]
    assert online_renderer.instances[0].kwargs["trust_request_chat_template"]
    assert serving_tokenization.instances[0].kwargs["trust_request_chat_template"]
    assert openai_serving_chat.instances[0].kwargs[
        "exclude_tools_when_tool_choice_none"
    ]
    assert online_renderer.instances[0].kwargs[
        "exclude_tools_when_tool_choice_none"
    ]
